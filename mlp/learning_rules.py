...
class AdamLearningRule(GradientDescentLearningRule):
    ...
    def reset(self):
        ...
        """
        for moms_1 in self.moms_1:
            moms_1 *= 0.
        
        for moms_2 in self.moms_2:
            moms_2 *= 0.
    ...
    def update_params(self, grads_wrt_params):
        ...
        for param, moms_1, moms_2, grad in zip(
                self.params, self.moms_1, self.moms_2, grads_wrt_params):            
            moms_1 *= self.beta_1
            moms_1 += (1-self.beta_1)*grad
            moms_2 *= self.beta_2
            moms_2 += (1-self.beta_2)*grad ** 2
            
            param -= (self.learning_rate * moms_1 /
                      (moms_2 + self.epsilon)**0.5)
...
class AdamLearningRuleWithWeightDecay(GradientDescentLearningRule):
    ...
    def __init__(self, learning_rate=1e-3, beta_1=0.9, beta_2=0.999, epsilon=1e-8, weight_decay=0.00001): 
    #def __init__(self, learning_rate=1e-3, initial_learning_rate=1e-3, beta_1=0.9, beta_2=0.999,
    ...
    def update_params(self, grads_wrt_params):
        ...
        self.params, self.moms_1, self.moms_2, grads_wrt_params):
            ...
            moms_1_hat = moms_1 / (1-self.beta_1**self.step_count)
            moms_2_hat = moms_2 / (1-self.beta_2**self.step_count)
            
            param -= eta * (self.initial_learning_rate * moms_1_hat /(moms_2_hat**0.5 + self.epsilon) + self.weight_decay*param)
...
class RMSPropLearningRule(GradientDescentLearningRule):
    ...
    def __init__(self, learning_rate=1e-5, beta=0.9, epsilon=1e-8):
        """Creates a new learning rule object.
        Args:
            learning_rate: A postive scalar to scale gradient updates to the
                parameters by. This needs to be carefully set - if too large
    ...
    def update_params(self, grads_wrt_params):
        ...
        Args:
            grads_wrt_params: A list of gradients of the scalar loss function
                ...
                previously, with this list expected to be in the same order.
        """
        for param, moms_1, grad in zip(
                self.params, self.moms_1, grads_wrt_params):
            moms_1 = self.beta * moms_1 + (1-self.beta)*grad**2
            param -= (self.learning_rate * grad /
                      (moms_1 + self.epsilon)**0.5)
