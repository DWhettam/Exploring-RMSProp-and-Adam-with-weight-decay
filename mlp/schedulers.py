...
class CosineAnnealingWithWarmRestarts(object):
    ...
    def __init__(self, min_learning_rate, max_learning_rate, total_iters_per_period, max_learning_rate_discount_factor,
        ...
        :param min_learning_rate: The minimum learnbing rate the scheduler can assign
    ...
    def update_learning_rule(self, learning_rule, epoch_number):
        ...
        """
        T_cur = epoch_number
        while T_cur >= self.total_epochs_per_period:
            T_cur -=  self.total_epochs_per_period                 
                
        if T_cur == (self.total_epochs_per_period -1):     
            self.total_epochs_per_period *= self.period_iteration_expansion_factor            
            
        eta = self.min_learning_rate + 0.5 * (self.max_learning_rate - self.min_learning_rate) * (1 + np.cos(T_cur/self.total_epochs_per_period * np.pi))
